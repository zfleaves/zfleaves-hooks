 ...
 * 键盘按下事件
 * @param [keyFilter: any] 当前键
 * @param [eventHandler: Function] 回调函数
 * @param [option: Object] 配置项
 * @returns void
 */
function useKeyPress(
    keyFilter: KeyFilter, // 要监听的按键或按键组合
    eventHandler: (event: KeyboardEvent, key: KeyType) => void, // 按键触发时的回调函数
    option?: Options, // 可选的配置项
) {
    // 解构配置项，设置默认值
    const { events = defaultEvents, target, exactMatch = false, useCapture = false } = option || {};
    // 使用useLatest保持回调函数的最新引用
    const eventHandlerRef = useLatest(eventHandler);
    // 使用useLatest保持keyFilter的最新引用
    const keyFilterRef = useLatest(keyFilter);

    // 使用useDeepCompareEffectWithTarget在目标元素上添加事件监听
